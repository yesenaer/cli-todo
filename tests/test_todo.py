...
def test_add(reset_data, test_data, runner):
    with patch("todo.todo.DATA", test_data):
        result = runner.invoke(app, ["add", "potions"])
        assert result.exit_code == 0
        assert result.stdout.strip() == "Added potions to your todo list."
        assert key_in_data(test_data, "item", "potions")
        assert key_in_data(test_data, "done", False)


def test_add_list_option(reset_data, test_data, runner):
    with patch("todo.todo.DATA", test_data):
        result = runner.invoke(app, ["add", "swords", "--list", "weapons"])
        assert result.exit_code == 0
        assert result.stdout.strip() == "Added swords to your weapons list."
        assert key_in_data(test_data, "item", "swords", "weapons")
        assert key_in_data(test_data, "done", False, "weapons")


def test_add_done_option(reset_data, test_data, runner):
    with patch("todo.todo.DATA", test_data):
        result = runner.invoke(app, ["add", "food", "--done"])
        assert result.exit_code == 0
        assert result.stdout.strip() == "Added food to your todo list."
        assert key_in_data(test_data, "item", "food")
        assert key_in_data(test_data, "done", True)
